...
class ConvBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, residual: bool = False):
        ...
        self.main = nn.Sequential(
            ...
            nn.GroupNorm(8, out_channels), # зачем это делать?
            nn.ReLU(),
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Residual работает только если у нас in_channels == out_channels
        x = self.main(x)
        if self.is_res:
            x = x + self.conv(x)
            return x / 1.414
...
class UnetModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        rep_times = down2.shape[2]
        temb = torch.unsqueeze(torch.unsqueeze(self.timestep_embedding(t), 2), 3)
        thro = self.up0(thro + temb)
        up1 = self.up1(thro, down3) + temb.repeat(1, 1, rep_times, 1)
